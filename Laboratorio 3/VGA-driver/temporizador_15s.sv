module temporizador_15s #(
    ...
    input  logic start,        // start que proviene la FSM 
    input  logic clear,        // mantiene el contador en 0 mientras esté en 1 
    output logic timeout,      // se alcanzó el límite (count_up == 15, display muestra 0)
    output logic pulso_1hz,    // pulso de 1 Hz para cambio de segundo
    ...
    //    SOLO se resetea con reset global 
    logic tick_1Hz;
    
    divisor_1hz_50m #(.TARGET(DIV_TARGET)) udiv (
        .clk_50m (clk_50m),
        .reset   (reset),  //  reset global (clear)
        .tick_1Hz(tick_1Hz)
    );
    ...
    always_ff @(posedge clk_50m or posedge reset) begin
        if (reset) begin
            ...
            // Contar cada 50 millones de ciclos 1 segundo
